...
function distubeStatus(queue, filters = false, autoPlay = false) {
  ...
  const filter = ` | Filter: \`${queue.filters.join(", ") || "Off"}\``;
...
class miPay {
  ...
  queryBalance(queryString) {
    ...
    this.#results = this.#fetchAPI(this.#dataBody)
      .then((res) => res)
      .catch((err) => err);
...
async function getPlaylistItems(playlistId, accessToken, pageToken = "") {
  ...
  try {
    ...
    console.error(
      "Error al obtener los elementos de la lista de reproducción:",
      error,
    );
  }
}
  ...
  do {
    ...
    if (data && data.items) {
      items = items.concat({
        ...
        contentDetails: data.items.contentDetails,
      });
